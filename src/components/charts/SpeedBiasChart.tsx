...
export function SpeedBiasChart({ results }: { results: PuttResult[] }) {
  const buckets = [
  ...
  const rows = buckets.map((b) => {
    ...
    const slowMisses = inBucket.filter(
      (r) => !r.made && r.speedMiss === 'too slow',
    ).length
    const fastMisses = inBucket.filter(
      (r) => !r.made && r.speedMiss === 'too fast',
    ).length
    const total = inBucket.length
    // Positive = fast bias, negative = slow bias
    const bias =
      total > 0
        ? Math.round(((fastMisses - slowMisses) / total) * 100)
        : 0
    return { ...b, total, slowMisses, fastMisses, bias }
  })
